...
const useOrbCanvas = (canvasRef, rootRef) => {
  ...
  const drawOrbs = useCallback(({ ctx, width, height }) => {
  ...
  const onDraw = useCallback(({ ctx, width, height, timestamp }) => {
    orbs.forEach((orb) => {
      orb.x += orb.vx / width;
      orb.y += orb.vy / height;
...
const useStarfieldCanvas = (canvasRef, rootRef) => {
  ...
  const drawStars = useCallback(({ ctx, width, height, timestamp, isStatic }) => {
    ctx.clearRect(0, 0, width, height);

    stars.forEach((star) => {
      const twinkle = isStatic
        ? 1
        : 0.55 + Math.sin(timestamp * star.speed + star.phase) * 0.25;
